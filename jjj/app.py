"""
Environmental Monitoring & Alert System - Flask Backend
...
def calculate_risk_metrics(weather_data):
    """Calculate additional risk metrics server-side"""
    ...
    # Calculate heat index if temperature is high
    ...
    if temp_c > 25:
        ...
        metrics['heat_index'] = round((hi - 32) * 5/9, 1)
    
    # Calculate wind chill if temperature is low
    if temp_c < 10:
        wind_kph = current.get('wind_kph', 0)
        if wind_kph > 4.8:
            wc = 13.12 + 0.6215*temp_c - 11.37*(wind_kph**0.16) + 0.3965*temp_c*(wind_kph**0.16)
            metrics['wind_chill'] = round(wc, 1)
    ...
    if air_quality:
        ...
        aqi = max(
            pm25 / 35 * 100,  # PM2.5 threshold
            pm10 / 50 * 100,  # PM10 threshold
            no2 / 200 * 100,  # NO2 threshold
            o3 / 180 * 100    # O3 threshold
        )
        metrics['air_quality_index'] = round(aqi, 0)
...
def get_weather():
    ...
    Query params: location (city name or coordinates)
    """
    try:
        ...
        if response.status_code != 200:
            ...
            return jsonify({
                'success': False,
                'error': error_data.get('error', {}).get('message', 'Failed to fetch weather data')
...
def search_locations():
    ...
    try:
        ...
        if response.status_code != 200:
            return jsonify({'error': 'Failed to search locations'}), response.status_code
...
@app.route('/api/alerts/contextual', methods=['POST'])
...
@app.route('/api/historical/<location>', methods=['GET'])
def get_historical(location):
    """
    Get historical weather data for a location
...
def predict_weather(location):
    ...
    try:
        ...
        if not historical or len(historical) < 3:
            return jsonify({
                'success': False,
                'error': 'Insufficient historical data for prediction. Need at least 3 readings.',
...
@app.route('/api/predict/<location>/multi', methods=['GET'])
def predict_multi_hour(location):
    """
...
@app.route('/api/analysis/<location>', methods=['GET'])
def analyze_location(location):
    ...
    try:
        ...
        if not historical:
            return jsonify({
                'success': False,
                'error': f'No data available for {location}'
            })
...
@app.route('/api/locations/monitored', methods=['GET'])
...
def extract_location_from_query(query, default_location='Mumbai'):
    ...
    location_patterns = [
        ...
        r'([a-zA-Z][a-zA-Z\s]+?)\'s\s+(?:air|weather|temperature)',
    ]
...
def generate_ai_response(user_query, weather_data, location):
    ...
    if any(word in query_lower for word in ['walk', 'jog', 'run', 'exercise', 'outdoor', 'outside', 'go out']):
        ...
        # Explanation query
        response['answer'] = f"📋 **Environmental Analysis for {location}:**\n\n"
        response['answer'] += f"Air quality is currently {air_quality['level']} due to PM2.5 levels at {pm25} µg/m³.\n\n"
        
        if correlations:
            response['answer'] += "🔍 **Key Factors:**\n"
            for corr in correlations:
                response['answer'] += f"\n**{corr['title']}** ({corr['type']})\n{corr['description']}\n"
        
        response['answer'] += f"\nCurrent risk score: {safety_analysis['safety_score']}/100"
    
    elif any(word in query_lower for word in ['air quality', 'aqi', 'pollution', 'pm2.5']):
        # Air quality specific query
        response['answer'] = f"🌫️ **Air Quality Report for {location}:**\n\n"
        response['answer'] += f"Status: {air_quality['level']} ({air_quality['color']})\n"
        response['answer'] += f"PM2.5: {pm25} µg/m³\n"
        response['answer'] += f"PM10: {pm10} µg/m³\n\n"
        response['answer'] += air_quality['description']
    
    elif any(word in query_lower for word in ['temperature', 'temp', 'hot', 'cold', 'weather']):
        # Weather query
        response['answer'] = f"🌡️ **Weather Conditions in {location}:**\n\n"
        response['answer'] += f"Temperature: {temp_c}°C\n"
        response['answer'] += f"Condition: {condition}\n"
        response['answer'] += f"Humidity: {humidity}%\n"
        response['answer'] += f"Wind: {wind_kph} km/h\n\n"
        
        if predictions:
            response['answer'] += f"Expected temperature in 1 hour: {predictions['temperature']}°C ({predictions['temperature_trend']})"
    
    else:
        # General query
        response['answer'] = f"📊 **Environmental Summary for {location}:**\n\n"
        response['answer'] += f"🌡️ Temperature: {temp_c}°C\n"
        response['answer'] += f"💧 Humidity: {humidity}%\n"
        response['answer'] += f"🌫️ Air Quality: {air_quality['level']} (PM2.5: {pm25} µg/m³)\n"
        response['answer'] += f"💨 Wind: {wind_kph} km/h\n"
        response['answer'] += f"🎯 Safety Score: {safety_analysis['safety_score']}/100\n\n"
...
def ai_query():
    """
    AI Assistant endpoint for intelligent environmental analysis
    
    Example request:
    POST /api/ai/query
    {
        "query": "Is it safe to go for a walk?",
        "location": "Mumbai"
    }
    """
    try:
        ...
        provided_location = data.get('location', None)
        
        # Extract location from the query itself
        detected_location = extract_location_from_query(user_query, default_location='Mumbai')
        
        # Use detected location unless a specific location was provided
        location = provided_location if provided_location else detected_location
        
        print(f"🤖 AI Query: '{user_query}'")
        print(f"📍 Detected location: '{detected_location}' | Using: '{location}'")
        
        # Handle casual conversation without location/weather data
        if location is None:
            print("💬 Casual conversation detected - responding without weather data")
            
            # Create friendly greeting response
            if any(word in user_query.lower() for word in ['hi', 'hello', 'hey']):
                greeting_response = "👋 Hello! I'm your environmental monitoring assistant. I can help you check:\n\n🌡️ Weather conditions\n🌫️ Air quality (PM2.5, PM10)\n🏃 Outdoor safety scores\n📈 Next hour predictions\n\nJust ask me about any city! For example: 'How is the air quality in Mumbai?' or 'Is it safe to go for a walk in Tokyo?'"
            elif 'help' in user_query.lower():
                greeting_response = "🤝 I'm here to help! I can provide real-time environmental data for any location.\n\nTry asking:\n• 'What's the weather in Manali?'\n• 'Is the air quality good in Delhi?'\n• 'Can I go jogging in Mumbai?'\n\nWhich location would you like to know about?"
            else:
                greeting_response = "👋 Hi there! Ask me about weather or air quality in any city!"
            
            # Try AI enhancement if available
            if USE_AI_API:
                casual_response = generate_ai_response_with_openai(user_query, None, None, None, None, None, None)
                # Only use AI response if it's not an error message
                if casual_response and not casual_response.startswith("🔑"):
                    greeting_response = casual_response
            
            return jsonify({
                'answer': greeting_response,
                'location': None,
                'mode': 'template_casual'
            }), 200
        
        # Fetch current weather data for the detected location
        url = f"{WEATHER_API_BASE}/current.json"
        params = {
            'key': WEATHER_API_KEY,
            'q': location,
        ...
        if response.status_code != 200:
            return jsonify({
                'error': f'Could not find weather data for "{location}". Please check the city name and try again.',
                'suggested_locations': ['London', 'Mumbai', 'New Delhi', 'New York', 'Tokyo', 'Paris', 'Manali', 'Goa']
            }), 404
        ...
        # Use actual location name from API response
        actual_location = location_data.get('name', location)
        
        print(f"✅ Fetched data for: {actual_location}")
        ...
        ai_response = generate_ai_response(user_query, weather_data, actual_location)
        
        print(f"✅ AI Response generated for '{actual_location}'")
...
if __name__ == '__main__':
    print("=" * 70)
    print("  Environmental Monitoring & Alert System - Backend Server")
    print("=" * 70)
